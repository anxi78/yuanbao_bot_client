...
def get_auto_reply(text, is_group=False):
    ...
    print(f"[自动回复调试] DEFAULT_REPLY: '{DEFAULT_REPLY}'")
    
    # 如果消息为空，直接返回默认回复
    if not text:
        print(f"[自动回复调试] 消息为空，直接返回默认回复")
        if DEFAULT_REPLY:
    ...
    for i, rule in enumerate(AUTO_REPLY_RULES):
        print(f"[自动回复调试] 规则{i}: match_type={rule.get('match_type')}, pattern={rule.get('pattern')}, group_only={rule.get('group_only', False)}")
    
    # 遍历规则，按顺序匹配
    for i, rule in enumerate(AUTO_REPLY_RULES):
        # 检查群聊限制条件
        if rule.get('group_only', False) and not is_group:
            print(f"[自动回复调试] 规则{i} 跳过: 仅限群聊，当前为{'群聊' if is_group else '私聊'}")
            continue
                    ...
                    if cmd_type == CMD_TYPE_RESPONSE:
                        ...
                        if inbound:
                            try:
                                ...
                                print(f"[消息提取] text_parts: {text_parts}, 合并后text: '{text}'")
                                print(f"[消息提取] bot_id: {self.bot_id}, is_at_me: {is_at_me}")
                                
                                # 去除消息开头的@符号和可能的机器人ID
                                original_text = text
                                if text and is_at_me:
                                    # 去除以@bot_id开头的部分
                                    bot_id_prefix = f"@{self.bot_id}"
                                    if text.startswith(bot_id_prefix):
                                        text = text[len(bot_id_prefix):].strip()
                                        print(f"[消息清理] 去除bot_id前缀: '{original_text}' -> '{text}'")
                                    # 去除以@开头的昵称（可能包含空格）
                                        ...
                                        if space_pos > 0:
                                            ...
                                            print(f"[消息清理] 去除@昵称前缀: '{original_text}' -> '{text}'")
                                        else:
                                            # 如果没有空格，说明是纯@符号或纯@昵称
                                            # 如果是纯@昵称，也视为纯艾特，将text设为@
                                            if text != '@':
                                                print(f"[消息清理] 纯@昵称视为纯艾特: '{original_text}' -> '@'")
                                                text = '@'
                                            # 否则是纯@符号，保持不变
                                
                                # 如果清理后文本为空，但确实艾特了机器人，视为纯艾特
                                if not text and is_at_me:
                                    print(f"[消息清理] 空文本但艾特了机器人，视为纯艾特: '@'")
                                    text = '@'
